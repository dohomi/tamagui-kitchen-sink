...
function LmSelect({
  ...
  return <import_LmFormFieldContainer.LmFormFieldContainer id={id} theme={colorVariant ? import_core.colormap[colorVariant] : themeName || void 0} error={error} required={required} labelProps={labelProps} label={label} size={rest.size} fullWidth={fullWidth} labelInline={labelInline} helperText={helperText} helperTextProps={helperTextProps}><import_tamagui.Select id={id} {...rest} value={selectVal} onValueChange={(val) => {
    setSelectVal(val);
    if (typeof onValueChange === "function") {
      onValueChange(val);
    }
  }}>
    <import_tamagui.Select.Trigger width={fullWidth ? "100%" : width} iconAfter={<import_tamagui_phosphor_icons.CaretDown />} paddingVertical={0} minHeight={rest.size}><import_tamagui.Select.Value placeholder={placeholder} paddingVertical={0} /></import_tamagui.Select.Trigger>
    ...
    <import_tamagui.Select.Content zIndex={2e5}>
      <import_tamagui.Select.ScrollUpButton alignItems="center" justifyContent="center" position="relative" width="100%" height="$3">
        <import_tamagui.YStack zIndex={10}><import_tamagui_phosphor_icons.CaretUp size={20} /></import_tamagui.YStack>
        <import_linear_gradient.LinearGradient start={[0, 0]} end={[0, 1]} fullscreen colors={["$background", "$backgroundTransparent"]} borderRadius="$4" />
      ...
      <import_tamagui.Select.ScrollDownButton alignItems="center" justifyContent="center" position="relative" width="100%" height="$3">
        <import_tamagui.YStack zIndex={10}><import_tamagui_phosphor_icons.CaretDown size={20} /></import_tamagui.YStack>
        <import_linear_gradient.LinearGradient start={[0, 0]} end={[0, 1]} fullscreen colors={["$backgroundTransparent", "$background"]} borderRadius="$4" />
